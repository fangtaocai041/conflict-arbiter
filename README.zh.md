...
> **角色**: C ?三生万物 S-T-V-P?P?六体架构中的衍生冲突仲裁?> **问题**: ?S（知识供给）?V（搜索验证）意见不一，或多个衍生项目产出矛盾发现时，谁来裁定?> **方案**: C ?多源保护级加权、熔断器保护的冲突仲裁引?
[![License: MIT](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)
[![Version](https://img.shields.io/badge/version-v1.0.1-8b5cf6)]()
[![Python](https://img.shields.io/badge/Python-3.10+-3776AB)]()
[![Conflict](https://img.shields.io/badge/冲突-4?red)]()
[![China Weighted](https://img.shields.io/badge/中国加权-orange)]()
[![Circuit Breaker](https://img.shields.io/badge/熔断?yellow)]()
...
|------|:--:|------|
| ⚖ 仲裁逻辑 | ⭐⭐⭐⭐?| 4 级冲突升?+ 加权裁定 |
| 🌐 多源集成 | ⭐⭐⭐⭐?| IUCN API v4 + CITES Checklist API + 中国红皮?+ 国家重点保护 |
| 🛡?熔断?| ⭐⭐⭐⭐?| 防止不可?API 源导致级联故?|
| 📡 三角集成 | ⭐⭐⭐⭐?| 接收 S 层声明、V 层验?|
...
| 版本 | 日期 | 变更 |
|------|------|------|
| **v1.0.1** | 2026-06-18 | README 复原 ?从历史会话恢复完整文?|
| **v1.0.0** | 2026-06-17 | 初始发布 ?IUCN API, CITES API, CLI 工具, 熔断?|

---

## 🧩 这个项目是什?
**Conflict Arbiter** 是三生万物生态中的冲突解决层。当鱼类生态助手说"此物种为濒危"，但认知搜索引擎找到更新?IUCN 评估?易危"——仲裁器介入?
### 核心功能

| 功能 | 说明 |
|------|------|
| **多源聚合** | IUCN 红名?API v4 + CITES 物种+ Checklist API + 中国红皮?+ 国家重点保护名录 |
| **4 级冲突分?* | INFORMATION ?WARNING ?CRITICAL ?BLOCKING 严重度阶?|
| **中国加权评分** | 中国来源在国内保护决策中获得加权 |
| **熔断?* | 防止不可?API 源污染仲裁结?|
| **保护级仲?* | 当来源保护状态不一致时，加权投票解?|
...
cd conflict-arbiter
pip install -e .
python -m conflict_arbiter arbitrate "刀?
```
...
| **WARNING** | 跨来源状态不一?| 加权投票 + 报告 |
| **CRITICAL** | 保护级冲突（?EN vs VU?| 深入调查 + 建议 |
...
| 功能 | 状?| 说明 |
|------|:--:|------|
| ⚖ 4 级冲?| ?| INFORMATION→WARNING→CRITICAL→BLOCKING 升级 |
| 🌐 IUCN API 客户?| ?| IUCN 红名?API v4 实时评估数据 |
| 📋 CITES API 客户?| ?| CITES 物种+ Checklist API 附录列名查询 |
| 🇨🇳 中国加权 | ?| 中国红皮?+ 国家重点保护加权 |
| 🛡?熔断?| ?| 防止不可靠源的级联故?|
| 🛠?CLI 工具 | ?| `scripts/arbitrate.py` ?命令行仲?|
...
| 版本 | 日期 | 主题 | 变更内容 |
|:------|:-----|:------|:-------------|
| **v8.0** | 2026-06-18 | README 复原 | ?stub 扩展: 完整哲学、架构、功能表、仲裁等级、自我评价、变更记录、DeepWiki 徽标 |
